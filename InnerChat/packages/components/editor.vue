...
export default {
  ...
  methods: {
      ...
      const defaultTools = [
        {
          ...
          click: null,
          render: (menu) => {
            return <i class='Drama-icon-emoji' />
          }
        },
        {
          name: 'uploadFile',
          ...
          render: (menu) => {
            return <i class='Drama-icon-folder' />
          }
        },
        {
          name: 'uploadImage',
          ...
          render: (menu) => {
            return <i class='Drama-icon-image' />
          }
        }
      ]
      let tools = []
    ...
    _renderEmojiTabs() {
      const renderImageGrid = (items) => {
        return items.map((item) => (
          <img
            src={item.src}
            title={item.title}
            class='Drama-editor__emoji-item'
      ...
      if (emojiData[0].label) {
        ...
        return <Drama-tabs style='width: 412px'>{nodes}</Drama-tabs>
      } else {
        return (
          <div class='Drama-tabs-content' style='width:406px'>
    ...
    _renderCustomerService() {
      ...
      const renderFn = (array) => {
        return array.map((item) => (
          <li
            class='Drama-editor__list-item'
            on-click={function() {
              console.log(`转接到客服(id:${item.id},name:${item.name}})`)
            }}
          >
            {item.name}
          </li>
      ...
      return <ul class='Drama-editor__list'>{renderFn(customerServiceArray)}</ul>
    },
  ...
  render() {
    ...
    this.proxyTools.forEach(({ name, title, render, click, isRight }) => {
      ...
      const classes = [
        'Drama-editor__tool-item',
        { 'Drama-editor__tool-item--right': isRight }
      ]
      let node
      if (name === 'emoji') {
        node =
          emojiData.length === 0 ? (
            ...
            <Drama-popover class='Drama-editor__emoji'>
      ...
      } else if (name === 'switchCustomerService') {
        node = (
          <Drama-popover class='Drama-editor__service'>
            <template slot='content'>{this._renderCustomerService()}</template>
            <div class={classes} title={title}>
              {render()}
            </div>
          </Drama-popover>
    ...
    return (
      <div class='Drama-editor'>
        {this.clipboardUrl && (
          <div class='Drama-editor__clipboard-image'>
            ...
            <div>
              ...
              <Drama-button on-click={this.sendClipboardImage}>发送图片</Drama-button>
            </div>
          </div>
        )}
        <input
        ...
        />
        <div class='Drama-editor__tool'>
          <div class='Drama-editor__tool-left'>{toolLeft}</div>
          <div class='Drama-editor__tool-right'>{toolRight}</div>
        </div>
        <div class='Drama-editor__inner'>
          <div
            class='Drama-editor__input'
            ref='textarea'
        ...
        <div class='Drama-editor__footer'>
          <div class='Drama-editor__tip'>
            {useScopedSlot(
              this.IMUI.$scopedSlots['editor-footer'],
              '使用 ctrl + enter 快捷发送消息'
            )}
          </div>
          <div class='Drama-editor__submit'>
            <Drama-button disabled={this.submitDisabled} on-click={this._handleSend}>
              {this.sendText}
            </Drama-button>
          </div>
        </div>
...
+b(Drama-editor) {
  ...
  +e(emoji) {
    ...
    .Drama-popover__content {
      padding: 0;
    }

    .Drama-popover__arrow {
      background: #f6f6f6;
    }
