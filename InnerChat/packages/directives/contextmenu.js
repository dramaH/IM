...
import DramaPopover from './../components/popover.vue'
let popover
    ...
    el.addEventListener(
      ...
      (e) => {
        ...
        if (!popover) {
          ...
          popover.className = 'Drama-contextmenu'
          ...
          .map((item) => {
            ...
            if (visible) {
              ...
              const icon = item.icon
                ? `<i class="Drama-contextmenu__icon ${item.icon}"></i>`
                : ''
              return `<div style="color:${item.color}" title="${item.text}" class="Drama-contextmenu__item">${icon}<span>${item.text}</span></div>`
            }
            return ''
          })
          .join('')
